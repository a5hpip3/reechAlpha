...
json.solutions do
  json.array! solutions do |solution|    
    ...
    if solution_is_purchased || current_user_is_solver || solver_friend_with_current_user
      ...
      if current_user_friend_with_question_owner || (!(link = question.linked_questions.where(user_id: current_user.reecher_id)).blank? || !(link = (current_user.friends & solution.wrote_by.friends).first).blank? || !(link = link_friends_to_question_owner_current_user).blank?)
