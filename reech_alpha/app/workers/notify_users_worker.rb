...
class NotifyUsersWorker
  ...
  sidekiq_options retry: false
    ...
    if !qust_details.nil?
      ...
      if (question_owner.has_email_notifications_enabled?("SOLUTION"))
      ...
      if (question_owner.has_device_notifications_enabled?("SOLUTION"))
        ...
        if !device_details.empty?
        	device_details.each do |d|
        		send_device_notification(d[:device_token].to_s, push_title ,d[:platform].to_s, push_title)
        	end
        end
      ...
      if !votings.blank?
      	votings.each do |v|
      		...
      		if starred_user.has_device_notifications_enabled?("STARANSW")
            ...
            if starred_user.has_email_notifications_enabled?("STARANSW")
